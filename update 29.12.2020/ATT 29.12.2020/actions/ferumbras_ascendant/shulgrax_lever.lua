	...
	if item.itemid == 9825 then
		...
		for y = 32785, 32789 do
			...
			if playerTile and playerTile:isPlayer() then
				if playerTile:getStorageValue(Storage.FerumbrasAscension.ShulgraxTimer) >= 1 then
		...
		for y = 32785, 32789 do
			...
			if playerTile and playerTile:isPlayer() then
				if verificarStorage < 1 then
					...
					playerTile:setStorageValue(Storage.FerumbrasAscension.ShulgraxTimer, os.time() + 6 * 3600))
				end
