	...
	if item.itemid == 9825 then
		...
		for y = 31660, 31664 do
			...
			if playerTile and playerTile:isPlayer() then
				if playerTile:getStorageValue(Storage.ForgottenKnowledge.TimeGuardianTimer) >= 1 then
		...
		for y = 31660, 31664 do
			...
			if playerTile and playerTile:isPlayer() then
				if verificarStorage < 1 then
					...
					playerTile:setStorageValue(Storage.ForgottenKnowledge.TimeGuardianTimer, os.time() + 6 * 3600)
				end
			end
		end		
