...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid == 9825 then
		...
		if player:getStorageValue(Storage.ForgottenKnowledge.HorrorTimer) > 0 then
		...
		for y = 31088, 31092 do
			...
			if playerTile and playerTile:isPlayer() then
				if playerTile:getStorageValue(Storage.ForgottenKnowledge.HorrorTimer) >= 1 then
		...
		for y = 31088, 31092 do
			...
			if playerTile and playerTile:isPlayer() then
				if verificarStorage < 1 then
					...
					playerTile:setStorageValue(Storage.ForgottenKnowledge.HorrorTimer, os.time() + 6 * 3600)
				end
