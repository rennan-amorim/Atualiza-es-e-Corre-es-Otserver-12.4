...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid == 9825 then
		...
		if player:getStorageValue(Storage.ForgottenKnowledge.LadyTenebrisTimer) > 0 then
		...
		for y = 31623, 31627 do
			...
			if playerTile and playerTile:isPlayer() then
				if playerTile:getStorageValue(Storage.ForgottenKnowledge.LadyTenebrisTimer) >= 1 then
		...
		for y = 31623, 31627 do
			...
			if playerTile and playerTile:isPlayer() then
				if verificarStorage < 1 then
					...
					playerTile:setStorageValue(Storage.ForgottenKnowledge.LadyTenebrisTimer, os.time() + 6 * 3600)
				end
